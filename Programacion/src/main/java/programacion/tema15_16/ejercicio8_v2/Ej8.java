...
public class Ej8 {
    public static void main(String[] args) {
        ...
        System.out.println("-------------------------------------");
        System.out.println("Numero de megustas en total: " + biblio.stream().mapToDouble(Libro::getMeGusta).sum());
        // f)
        System.out.println("-------------------------------------");
        List<Libro> biblioUsa = biblio.stream().peek(s1 -> s1.setPrecio(s1.getPrecio() / 0.92f))
                .collect(Collectors.toList());
        biblioUsa.stream().forEach(System.out::println);
        // g)
        System.out.println("-------------------------------------");
        System.out.println(
                "Libro con mas me gustas: " + biblio.stream().max((s1, s2) -> s1.getMeGusta() - s2.getMeGusta()).get());
    }
}
