...
/**
 * Clase Empleado desarrollo que representa a los empleados que so desarrolladores de una empresa, hereda de la clase empleado.
 */
public class EmpleadoDesarrollo extends Empleado implements Comparable<EmpleadoDesarrollo>{
    /**
     * Esta es un atributo que sera la especialidad del empleado, viene de una enumeracion Especialidad
     */
    private Especialidad especialidad;

    /**
     * Este es un atributo de tipo array de strings que almacenara los proyectos realizados por los empleados
     */
    private String[] proyectosRealizados;

    /**
     * Constructor que recibe todos los parámetros del padre más la especialidad y un array de proyectos.
    ...
    /**
     * Constructor adicional que use un valor por defecto de especialidad (DESARROLLO) y un array de proyectos vacío. 
    ...
    /**
     * Metodo calcular salario que calculara el salario del empleado segun su especialidad
     * @return salario
     */
    @Override
    public double calcularSalario() {
        if(especialidad.equals(Especialidad.DESARROLLO)) return 3000;
        else if(especialidad.equals(Especialidad.DISEÑO))return 2800;
        else if(especialidad.equals(Especialidad.INFRAESTRUCTURA)) return 3200;
        else if(especialidad.equals(Especialidad.GESTION)) return 3500;
        else return 0;
    }

    /**
     * Metodo que obtiene los n proyectos realizados mas complejos, por complejo nos referimos al tamaño del nombre del proyecto
    ...
    public String[] obtenerProyectosMasComplejos(int n){ // Se ha intentado
        ...
        if(proyectosRealizados.length == 0) return new String[0];
        ...
        if(proyectosRealizados.length>proyectosComplejos.length){
            while (contador < n) {
                ...
                for (int i = 0; i < proyectosRealizados.length; i++) {
                    ...
                    if(siguiente.length()>proyectosRealizados[masGrande].length()) masGrande=i;
            ...
            for (int i = 0; i < proyectosRealizados.length; i++) {
                ...
                proyectosRealizados[i] = proyectosRealizados[masGrande];
            }
            return proyectosRealizados;
        }
    ...
    /**
     * Metodo heredado de la interfaz comparable, habiendo sido declarada la implementacion en la clase padre (Empleado)
    ...
    public int compareTo(EmpleadoDesarrollo e) {
        ...
        if(nombre ==0) return (int) (this.calcularSalario() - e.calcularSalario());
